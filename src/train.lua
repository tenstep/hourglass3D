...
if opt.saveHeatmaps then saved.heatmaps = torch.Tensor(validSamples, unpack(ref.outputDim[2])) end
...
function step(tag)
    ...
    if tag == 'train' then
        ...
        if tag == 'predict' then
            ...
            if opt.saveHeatmaps then saved.heatmaps = torch.Tensor(nSamples, unpack(ref.outputDim[2])) end
            set = 'test'
        else
            set = 'valid'
        end
    ...
    for i,sample in loader[set]:run() do
        ...
        if tag == 'train' then
            ...
            if math.fmod(epoch,opt.step_size) == 0 then
                optimState['learningRate'] = optimState['learningRate'] / opt.lr_decay
            end
