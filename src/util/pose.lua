...
joint_sigma = {0.1,1.07,0.87,0.89,1.07,0.87,0.89,0.1,0.1,0.1,0.1,0.79,0.72,0.62,0.79,0.72,0.62}
...
function generateSample(set, idx)
    ...
    for i = 1,dataset.nJoints do
        if pts[i][1] > 1 then -- Checks that there is a ground truth annotation
            drawGaussian(out[i], transform(pts[i], c, s, r, opt.outputRes), joint_sigma[i])  -- use different variances for different joints
        end
    end
...
function loadData(set, idxs)
    ...
    local input,label,label_lowres
