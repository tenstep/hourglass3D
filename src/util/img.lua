...
function drawGaussian(img, pt, sigma)
    ...
    local tmpSize = 2 --math.ceil(3*sigma)
    ...
    local g = image.gaussian(size,sigma * 0.2)
...
function Gaussian3D(size,sigma)
    local res = torch.zeros(size,size,size)
    local center = (size+1)/2
    for i = 1,size do
        for j = 1,size do
            for k = 1,size do
                res[i][j][k] = torch.exp(-((i-center)*(i-center)+(j-center)*(j-center)+(k-center)*(k-center))/2/sigma/sigma)
            end
        end
    end
    return res
...
function drawGaussian3D(img, pt, sigma)
    -- Draw a 3D gaussian
    -- Check that any part of the gaussian is in-bounds
    local tmpSize = 2 --math.ceil(3*sigma)
    local ul = {math.floor(pt[1] - tmpSize), math.floor(pt[2] - tmpSize),math.floor(pt[3] - tmpSize)}
    local br = {math.floor(pt[1] + tmpSize), math.floor(pt[2] + tmpSize),math.floor(pt[3] + tmpSize)}
    -- If not, return the image as is
    if (ul[1] > img:size(2) or ul[2] > img:size(1) or ul[3] > img:size(3) or br[1] < 1 or br[2] < 1 or br[3] < 1) then return img end
    -- Generate gaussian
    local size = 2*tmpSize + 1
    local g = Gaussian3D(size,sigma)
    -- Usable gaussian range
    local g_x = {math.max(1, -ul[1]), math.min(br[1], img:size(2)) - math.max(1, ul[1]) + math.max(1, -ul[1])}
    local g_y = {math.max(1, -ul[2]), math.min(br[2], img:size(1)) - math.max(1, ul[2]) + math.max(1, -ul[2])}
