...
projectDir = '/media/yuyin/Work_4T/chenxu/Hourglass/torch/hg3D'
datasetDir = '/media/yuyin/Work_4T/chenxu/pose_dataset/human36m/proto2'
...
local function parse(arg)
    ...
    cmd:option('-dataset',        'h36m', 'Dataset choice: mpii | flic')
    cmd:option('-dataDir',  datasetDir , 'Data directory')
    ...
    cmd:option('-validIters',        493, 'Number of validation iterations per epoch')
    cmd:option('-validBatch',          6, 'Mini-batch size for validation')
    cmd:option('-nValidImgs',       2958, 'Number of images to use for validation. Only relevant if randomValid is set to true')
    cmd:option('-step_size',          25, 'decay the learning rate after step_size iterations (step policy)')
    cmd:option('-lr_decay',           10.0, 'step learning rate decay')
    ...
    opt.dataDir = paths.concat(opt.dataDir)
